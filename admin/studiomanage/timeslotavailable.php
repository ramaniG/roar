...
if($dates != "" && $monthId != "")
{
	...
	foreach($dateArray as $dateslot)
	{
		...
		if ($studio == 3)
		{
			addavailable($newdate->format('Y-m-d'), $timeslotid, 1);
			addavailable($newdate->format('Y-m-d'), $timeslotid, 2);
		}
		else
		{
			addavailable($newdate->format('Y-m-d'), $timeslotid, $studio);
		}
	}
...
function addavailable($date, $timeslotid, $studioid)
{
	$db = new Database();
	...
	if ($inclose != null && mysql_num_rows($inclose) > 0) {
		$sql = "DELETE FROM timeslot_notavailable WHERE date='".$date."' AND studioid=$studioid AND timeslotid=$timeslotid";
	...
	if ($duperaw != null && mysql_num_rows($duperaw) == 0) {
		$sql = "INSERT INTO timeslot_available (date, timeslotid, studioid) VALUES ('".$date."', $timeslotid, $studioid)";
