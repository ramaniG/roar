...
class FBGallery
{
	public function __construct($id,$breadcrumbs,$cache=array(), $in)
	{
		/**
	...
	function getData($id,$type='')
	{
		...
		*/
		if(!empty($id))
		{
			...
			else if($type == 'description'){$query = "SELECT description FROM album WHERE aid = '$id'";}
			else{$query = "SELECT aid,object_id,name,size,type,created FROM album WHERE owner = '$id' $this->in ORDER BY created DESC";}
		...
		for($x=0; $x<$data_count; $x++)
		{
			if(!empty($json_array['data'][$x]['object_id']) AND $json_array['data'][$x]['size'] > 0) // do not include empty albums
			{
				$gallery .= '<li class="span2">
    							<a href="?id='.$json_array['data'][$x]['aid'].'&title='.urlencode($json_array['data'][$x]['name']).'" class="thumbnail" rel="tooltip" data-placement="bottom" >
    							<img src="http://graph.facebook.com/'.$json_array['data'][$x]['object_id'].'/picture?type=album">
    							</a>
    							<div class="roarfont1">'.$json_array['data'][$x]['name'].'</div>
    							<div class="roarfont2">'.$json_array['data'][$x]['size'].' photos</div> 						
  							</li>';
			}
	...
	function displayPhotos($album_id,$title='Photos')
	{
		...
		$json_array2 = $this->getData($album_id,$type='description');
		$data_count = count($json_array['data']);
		if($data_count > 0)
		{
			for($x=0; $x<$data_count; $x++)
			{
				$gallery .= '<div class="roarfont3">'.$json_array2['data'][$x]['description'].'</div> <li>
	...
	function addBreadCrumbs($crumbs_array)
	{
		...
		*/
		...
		{
			$divider = ' <span class="divider">/&nbsp</span>';
			...
			foreach($crumbs_array as $title => $url)
			{
				...
				$crumbs .= '<li class="roartitle"><a href="'.$url.'">'.stripslashes($title).'</a>'.$divider.'</li>';
				++$counter;
			}
